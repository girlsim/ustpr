...
class BasicAI(pygame.sprite.Sprite):
	...
	self.current_fire_pattern = 0

	self.firing = False

	self.speed, self.speed_max, self.speed_factor, self.angle_max, self.angle, self.angle_factor, score_a = self.move_ai_data
	self.deathBlow, self.fire_max, self.fire_data, score_b, self.explode_data = self.fire_ai_data
	self.firing_interval = self.fire_data[self.current_fire_pattern][7]
	self.firing_base_degree = self.fire_data[self.current_fire_pattern][8]
	self.firing_degree_passed = self.firing_base_degree
	self.firing_max = self.fire_data[self.current_fire_pattern][0]
	self.switch_back_point = self.fire_data[self.current_fire_pattern][1]
	...
	if self.firing == True:
		...
		self.firing_timer = 0
		self.firing_timer_old = 0
		self.current_fire_pattern += 1
		if self.current_fire_pattern > (len(self.fire_data) - 1):
		    self.current_fire_pattern = 0
		self.firing_interval = self.fire_data[self.current_fire_pattern][7]
		self.firing_base_degree = self.fire_data[self.current_fire_pattern][8]
		self.firing_degree_passed = self.firing_base_degree
